...
function ManageProducts() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`https://shopnex-server.vercel.app/products/user/${user.email}`);
        const data = await res.json();

        console.log('Fetched products:', data); // Debug: check what comes from backend

        // Backend response could be array or object with .products
  ...
  const handleDelete = async () => {
    try {
      await fetch(
        `https://shopnex-server.vercel.app/product/${selectedId}?email=${user.email}`,
        { method: 'DELETE' }
      );
